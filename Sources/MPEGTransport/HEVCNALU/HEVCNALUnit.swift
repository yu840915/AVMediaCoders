...
public struct HEVCNALUnitHeader: Equatable {
  ...
  public init(type: HEVCNALUnitType, layerID: UInt8, temporalIDPlus1: UInt8) {
    self.type = type
  ...
  init(bytes: [UInt8]) throws {
    guard bytes.count >= 2 else {
      throw MPEGTransportError.invalidHEVC(.invalidNALUnitHeaderLength)
    }
    let byte0 = bytes[0]
    guard byte0 & 0b10000000 == 0 else {
      throw MPEGTransportError.invalidHEVC(.nonZeroForbiddenBit)
    }
    let byte1 = bytes[1]
    guard let type = HEVCNALUnitType(rawValue: byte0 >> 1) else {
      throw MPEGTransportError.invalidHEVC(.invalidNALUnitType)
    }
    self.type = type
...
public struct HEVCNALUnit {
  ...
  public init(header: HEVCNALUnitHeader, payload: [UInt8]) {
    self.header = header
