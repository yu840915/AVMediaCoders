...
struct TSProgramAssociationSectionTests {
  ...
  func keepJustEnoughtBufferLength() async throws {
    let bytes: [UInt8] = [
      0x00, 0x80, 0x09, 0x00, 0x00, 0x01, 0x00, 0x00, 0x46, 0xC8, 0x17, 0x1B,
      0x00, 0x80, 0x09,
    ]
  ...
  func rejectInvalidPID() async throws {
    let bytes: [UInt8] = [
      ...
      0x16, 0xDC, 0x15, 0x9E,  //CRC
    ]

    #expect(throws: AVMediaCodersError.invalidTS(.unexpectedPID)) {
      try TSProgramAssociationSection(bytes: bytes)
    }
  }
  ...
  func sectionNumberShouldNotBeGreaterThanLastSectionNumber() async throws {
    let bytes: [UInt8] = [
      ...
      0x00,  //last section number
      0x00, 0x01, 0x00, 0x20, 0x00, 0x02, 0x00, 0x21, 0x14, 0xC0, 0x6D, 0x61,
    ]
